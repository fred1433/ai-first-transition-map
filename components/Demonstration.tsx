...
import type { AuditEntry, DailyLog, Proposal } from "@/lib/prototype/types";
...
export function Demonstration({ scenario, initialLog }: Props) {
  ...
  const acceptedHere = proposal?.acceptance?.contentHash === proposal?.contentHash;
      ...
      <div className="rounded-xl border border-rule bg-paper-raised p-6 md:p-8">
        ...
        <div className="mt-7 flex flex-wrap items-center gap-3">
          ...
          <span className="text-[13px] text-ink-faint">
            The recorded run calls nothing. A new one calls the model, under a daily cap.
          </span>
        </div>
      </div>
      ...
      {proposal && replay ? (
        <div className="mt-6 rounded-xl border border-rule bg-paper-raised p-6 md:p-8">
          ...
          <div className="mt-6 space-y-6">
            {proposal.fields.map((field) => (
              <div key={field.field}>
                ...
                {field.withheld ? (
                  <p className="mt-2 text-[13px] text-bronze">
                    Held back, not applied. {field.withheld.detail}
                  </p>
                ) : (
                  <p className="mt-2 text-[13px] text-ink-faint">
                    From the notes: {field.provenance.map((entry) => `"${entry.quote}"`).join("  ")}
                  </p>
                )}
              </div>
            ))}
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3 border-t border-rule pt-6">
            ...
            >
              {acceptedHere ? "Accepted by Dana" : "Accept as Dana, project manager"}
            </button>
            <button
              type="button"
              className={SECONDARY}
            ...
            <span className="text-[13px] text-ink-faint">
              The acceptance names version {proposal.contentVersion} of this draft. The server checks that, not the button.
            </span>
          </div>
        </div>
      ) : null}
        ...
        <div className="mt-6 rounded-xl border border-rule bg-paper-sunk p-6 md:p-8">
          ...
          {audit.length > 0 ? (
            <div className="mt-6 border-t border-rule pt-5">
              ...
              <ul className="mt-3 space-y-1 font-mono text-[12px] text-ink-faint">
